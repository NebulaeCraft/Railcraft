...
## 四次调整：无动力车辆在斜线上恢复自然减速

后续复测发现，斜线处理器只排除了完全静止的车辆，却会把任何仍有微小惯性的车辆重新归一化到 `0.6`。因此，无动力车辆每 tick 已经受到的原版阻力都会在世界 tick 末尾被抵消，车辆进入连续长斜线后便无法自然停下。

修复后，斜线处理分为两种情况：

- 整列联挂车辆中存在运行中的机车时，继续把斜线合速度维持在 `0.6`，并继续临时覆盖列车软速度帽。动力判定遍历整列车，不依赖机车位于矿车之前或之后，因此前拉和后推都保留原行为。
- 整列车中没有运行中的机车时，不再补充阻力损失；低于 `0.6` 的速度原样保留，让原版矿车阻力和既有联挂阻力逐 tick 生效。若惯性速度高于 `0.6`，仍只向下裁剪到强化轨道上限，避免斜线合速度超限。

本次没有修改 `LinkageHandler` 的弯道判定、弹簧或阻尼公式。长斜线上的平行车辆仍使用完整的直线联挂物理，真正跨越方向变化的车钩仍使用弯道修正。
...
- 无动力车辆低于斜线速度上限时不会被重新加速，可以继续受到自然阻力。
- 存在运行中机车的列车在联挂阻力之后仍会恢复斜线目标速度；动力判定覆盖前拉和后推编组。
